...
void PicFrame(Mat &src, vector<Rect> &rectFrame)
{
...
void PicFrame(Mat &src, Rect &rectFrame)
{
	cv::rectangle(src, rectFrame, Scalar(0, 0, 255), 8);
}

void PicDifferFrame(Mat &src, Rect &rectFrame1, Rect &rectFrame2)
{

}

void PicDifferFrame(Mat &src, vector<Rect> &rectFrameShiJue, vector<Rect> &rectFrameClient)
{
	int TH = 20;
	int flag = 1;
	...
	vector<Rect> &textTingJianGengDuo, vector<Rect> &picBottonIcon)
{
	...
	const Rect rectPicSearch(Point(0, 40), Point(width, 145));		//1.搜索框
...
void main()
{
	...
	PicDifferFrame(srcImg2, picSearch, picSearch_Client);
	...
	imwrite("Compare.jpg", srcImg2, compression_params);
	imwrite("Client.jpg", srcImg3, compression_params);
